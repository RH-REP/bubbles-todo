/* きっかけ／すきま に共通の「カードの一覧」画面（利用者の指示）

   もとは画面3「きっかけ」だけの実装だった。利用者の指示で
   すきまも同じUIにすることになったので、**アンカーに直に触っていたところを
   設定（CFG）ごしに呼ぶ形へ剥がした**。中身は1つ、設定が2つ。
   別々に書くと、片方だけ直したときに「基本的に同じUI」が崩れていく。

   カード（きっかけ＝アンカー / すきま＝枠）は見出しであって、記録の対象ではない。
   押しても何も起きない。中のバブルは一定サイズ（契約 §4）。
   いちばん下が「未分類」——この画面に来たが、まだどのカードにも入っていないもの。
   面には重力があるので、どこにも掴まれていないものが下に溜まる（追補5 §2）。

   ・入れたものは日をまたいでも消えない（立てっぱなしの計画）
   ・同じカードの中に上下関係は作らない（「先頭が主役」は廃止）
   ・セルの幅はバブルの直径の整数倍。バブルはその格子に乗る（端数を出さない）

   設定（CFG）で変わるのは：名前と文言、store のどの軸を触るか、
   日にち（きっかけだけ）を持つかどうか。CSS の名前（.plan-*）は共通のまま——
   見た目が同じであることを、クラス名の共有そのもので担保する。

   操作:
     ・タップ → 中央へ寄る → 盤の [まずは開始] … 5分だけの集中（bubble.js が見る）
     ・長押し … 詳細 / 5分だけ集中 / はじめた / 完了 / 消す
     ・見出しの左の取っ手をドラッグ … カードの並べ替え（⋮ の「上へ／下へ」も残す）
     ・カードからカードへドラッグ … 付け替え。未分類へ戻すとカードから外れる
     ・どのカードでもないところで離す … 未分類へもどる（トースト＋取り消し）
     ・タブへドラッグ … 海=全解除 / 今日=today を足す / ほか=その軸を足す（契約 §2） */
...
/* この画面が触る「軸」。きっかけならアンカー、すきまなら枠。
   ここを通さずに store のアンカー関数を直に呼ばないこと——呼んだ瞬間に
   すきまの画面だけ壊れる（片方でしか動かない道ができる）。 */
export function makeCardScreen(CFG) {
const A = CFG.axis;
/* 「はじめた」をどのキーで記録するか。きっかけはカードごと、すきまはカード無し（null）。
   すきまの枠は「いつやるか」ではなく「どんな状況か」なので、
   枠ごとに着手を数え分ける意味が無い（数え分けると同じ着手が枠の数だけ増える）。 */
const startKey = (cardId) => (CFG.startPerCard ? (cardId || null) : null);
...
function isStarted(id, anchorId) {
  return typeof store.isStarted === 'function' && !!store.isStarted(id, startKey(anchorId));
}
...
function makeDetail(todo, anchorId, key) {
  ...
  doneBtn.addEventListener('click', ev => {
    ev.stopPropagation();
    if (isStarted(todo.id, anchorId)) store.unstart(todo.id, startKey(anchorId));
    else store.start(todo.id, startKey(anchorId));
    syncDone();
  });
...
function openItemMenu(todo, anchor, key, node) {
  ...
  openBubbleMenu(node, {
    ...
    onStarted() { store.start(todo.id, startKey(anchor ? anchor.id : null)); },
...
function dropToTab(todo, tabId) {
  const id = todo.id;
  if (tabId === 'sea') {
    /* **軸を全部外す。**この画面の軸だけでなく、もう片方（きっかけ／すきま）も。
       「海へ戻す」は文字どおり、どこにも属していない状態へ戻すこと */
    const t = store.get(id) || {};
    const before = {
      today:   !!t.today,
      gap:     typeof store.isGap === 'function' ? !!store.isGap(id) : !!t.gap,
      plan:    typeof store.isPlan === 'function' ? !!store.isPlan(id) : !!t.plan,
      anchors: typeof store.anchorsOf === 'function' ? store.anchorsOf(id).slice() : [],
      slots:   typeof store.gapSlotsOf === 'function' ? store.gapSlotsOf(id).slice() : [],
    };
    store.setToday(id, false);
    store.setGap(id, false);
    if (typeof store.setPlan === 'function') store.setPlan(id, false);
    if (typeof store.clearAnchors === 'function') store.clearAnchors(id);
    if (typeof store.clearGapSlots === 'function') store.clearGapSlots(id);
    ...
    toast('「' + todo.text + '」を海へ戻した', {
      ...
      on: () => {
        ...
        if (before.plan && typeof store.setPlan === 'function') store.setPlan(id, true);
        if (typeof store.setAnchor === 'function') {
          before.anchors.forEach(aid => store.setAnchor(id, aid, true));
        }
        if (typeof store.setGapSlot === 'function') {
          before.slots.forEach(sid => store.setGapSlot(id, sid, true));
        }
      },
    });
    return;
  ...
  /* いま居る画面のタブへ落としても、何も足さない（もう居るので） */
  if (tabId === CFG.id) return;
  if (tabId === 'gap' && typeof store.setGap === 'function') {
  ...
  if (tabId === 'plan' && typeof store.setPlan === 'function') {
    store.setPlan(id, true);
    toast('「' + todo.text + '」をきっかけにも入れた', {
      label: '元に戻す', on: () => { store.setPlan(id, false); },
    });
  }
}
...
function strayDrop(id, from) {
  ...
  const before = A.membersOf(id).slice();
  ...
  toast('「' + t.text + '」を未分類へもどした', {
    label: '元に戻す',
    on: () => { before.forEach(aid => A.setMember(id, aid, true)); },
  });
}
...
function toUnsorted(id, from) {
  ...
  toast(label ? '「' + label + '」から外した' : CFG.word + 'から外した', {
    label: '元に戻す',
    on: () => { before.forEach(aid => A.setMember(id, aid, true)); },
  });
}
...
function openAnchorMenu(a, index, total, btn, host) {
  ...
  const rows = [
    ...
    /* 日にちはきっかけだけ。すきまには**行そのものを出さない**——
       押せない的を置くと、押せる的と見分けがつかない。
...
function removeAnchor(a, index) {
  const items = A.itemsIn(a.id);
  const ids = items.map(t => t.id);
  const name = a.name;
  const total = A.list().length;
  if (!A.remove(a.id)) return;
  ...
  toast('「' + name + '」を消した（' + ids.length + '件が入っていた）', {
    ...
    on: () => {
      ...
      ids.forEach(tid => A.setMember(tid, na.id, true));
      /* 追加は末尾に付くので、元の位置まで押し上げる */
      for (let i = total - 1; i > index; i--) A.move(na.id, -1);
    },
  }, 6000);
  render();
}
...
function openSchedule(a) {
  ...
  function finish() {
    ...
    if (!CFG.schedule || typeof store.anchorDue !== 'function' || store.anchorDue(a.id)) return;
    const lb = scheduleLabel(A.get(a.id) || a) || '毎日';
...
function syncAllBtn() {
  ...
  let any = false;
  try { any = CFG.schedule && A.list().some(a => (a.days || []).length); } catch (e) { any = false; }
  allBtn.hidden = !any;
...
function makeGrip(a, index, total) {
  ...
  g.addEventListener('keydown', ev => {
    ...
    ev.preventDefault();
    wantFocus = 'grip:' + a.id;
    if (!A.move(a.id, d)) wantFocus = null;   /* 端では動かない */
  });
...
function onReorderUp() {
  ...
  if (!A.move(st.id, delta)) { wantFocus = null; render(); }
}
...
function commitRename(id, value) {
  ...
  wantFocus = 'amenu:' + id;
  if (v && v !== nameOf(id)) {
    if (!A.rename(id, v)) { toast('その名前にはできない'); }
  }
  render();
}
...
function makeAnchorFrame(a, index, total, special) {
  ...
  if (renaming === a.id) {
    ...
    inp.setAttribute('aria-label', CFG.word + 'の名前');
...
/* 入力欄で選んだタグ。送ったあとも残す（同じタグのものを続けて書くのが楽）。
   隠し持たない——札は出たままなので、何が付くかは目で見える（今日の画面と同じ） */
const picked = new Set();

/* 札に出すタグ。**完了**は出さない（専用の道がある）。
   **この画面の軸**も出さない——ここで書いた時点で必ず付くので、選ばせる意味が無い */
function composerTags() {
  if (typeof store.tags !== 'function') return [];
  try {
    return store.tags().filter(t => t && t.id !== 'done' && t.id !== CFG.axisTag);
  } catch (e) { return []; }
}

function renderComposerTags(host) {
...
return {
  ...
  mount(node) {
    ...
    /* CSS の名前は共通（.plan-*）。画面ごとの差はこの1枚だけで付ける
       ——同じ見た目であることを、クラス名の共有そのもので担保する */
    wrap.classList.add('plan-' + CFG.id);
    ...
    hintEl = el('p', 'plan-hint', CFG.hint);
    hintEl.hidden = true;
    ...
    addBtn.appendChild(el('span', null, CFG.word + 'を足す'));
    ...
    addInput.type = 'text';
    addInput.placeholder = CFG.namePlaceholder;
    addInput.autocomplete = 'off';
    addInput.dataset.fk = 'newanchor';
    addInput.hidden = true;
    addInput.setAttribute('aria-label', CFG.word + 'の名前');
